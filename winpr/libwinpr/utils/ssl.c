...
#if defined(OPENSSL_VERSION_MAJOR) && (OPENSSL_VERSION_MAJOR >= 3)
	/* The legacy provider is needed for MD4 and RC4. When both are compiled in
	 * internally (WITH_INTERNAL_MD4 / WITH_INTERNAL_RC4) we don't need it and
	 * skip the load to avoid a misleading warning. */
